...
/**
 * Create a compacted context from a given fact name or context. Removes the trailing term if the parameter is 
 * a fact. Fact does not end with a '/' but a context does.
 * 
 ...
 * Examples: 
 * 
 *  '/a/given/../context/./or/../path' is compiled to '/a/context/'
 * 
 * @param {String} path The path or context to compile into a context.
 */
function compileContext(path) {
    let compiled = compilePath(path);
    return compiled.replace(trailingTermRemovalRegex, "");
