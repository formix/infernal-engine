...
class InfernalEngine {
    ...
    async get(path) {
        ...
        return Promise.resolve(this._facts.get(compiledpath));
    }
    ...
    async infer() {
        ...
        while (depth < this._inferenceDepth && this._agenda.size > 0) {
            ...
            currentAgenda.values().forEach(rulectx => {
                await rulectx.execute();
            });
            depth++;
        }
        if (generationCount == this._inferenceDepth) {
     ...
     * arrays as exploded arrays. Once the object is imported, launches the inference if specified.
     * 
