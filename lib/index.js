 ...
 * assumes the currentContext to be "/". Within a rule, the currentContext
...
function getHomePath(currentContext, path) {
    var context = getContext(currentContext, path);
...
function getContext(currentContext, path) {
    ...
    if (!currentContext || currentContext.indexOf("/") !== 0) {
        throw new Error("currentContext must start with a '/'. Value: '" + 
            currentContext + "'");
    }
    
    if (path.indexOf("/") === 0) {
        // Since "path" starts with a '/' means that "path" is absolute. We 
        // don't care about the currentContext in that case.
        return getContext(path, "");
    }

    var currentContextArray = currentContext.split("/");
    var pathArray = path.split("/");
    var contextStack = currentContextArray.concat(pathArray);
