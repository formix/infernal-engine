...
module.exports = class InfernalEngine {
    ...
    async set(path, value) {
        ...
        // rule is faster.
        if (!(value instanceof Array) && utils.equals(prevValue, value)) return;

        // Create or update the change entry.
        if (!this._changes.has(compiledpath)) {
            this._changes.set(compiledpath, new Change(compiledpath, prevValue, value));
        }
        else {
            this._changes.get(compiledpath).CurrentValue = value;
        }
    ...
    async infer() {
        ...
        this._busy = true;
        let depth = 0;
        this._facts.set("/$/maxDepth", this._maxDepth);

        try {
            while (depth < this._maxDepth && this._agenda.size > 0) {
                depth++;
                this._facts.set("/$/depth", depth);
                this._logger.log("debug", {
