...
Infernal.prototype.get = function(factPath) {
    var absolutePath = getAbsolutePath(this.scopes[0], factPath);
...
Infernal.prototype.addRule = function(rulePath, rule) {
    ...
    while (match) {
        var homePath = getHomePath("/", rulePath);
        factPath = getAbsolutePath(homePath, match[1]);
...
function getHomePath(base, path) {
    var absolute = getAbsolutePath(base, path);
    var lastSlash = absolute.lastIndexOf("/");
    return absolute.substring(0, lastSlash + 1);
}
