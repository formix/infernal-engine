...
InfernalEngine.prototype.set = function(factName, value, callback) {
    ...
    if (!utils.equals(oldValue, value)) {
        ...
        addToAgenda.call(this, factName);
...
InfernalEngine.prototype.addRule = function(ruleName, rule) {
    ...
    let ruleParameters = parseParameters(rule);
    
    // matcher for parameters with annotations: /*@ ../fact1 */ a1, /*@ fact2 */ b, c, d
    let regex = /(?:\/\*?@ *([\w/.]+?) *\*\/ *\w+,?)/g;
    let match = regex.exec(ruleParameters)
    this._rules[ruleName] = {
        run: rule,
        inputFacts: []
    };

    let context = utils.getContext("/", ruleName);
    while(match) {
        let factName = utils.getFullName(context, match[1] || match[2]);
        if (this._relations[factName] === undefined) {
            this._relations[factName] = {};
        }
        this._relations[factName][ruleName] = true;
        this._rules[ruleName].inputFacts.push(factName);
        match = regex.exec(ruleParameters);        
    }
...
InfernalEngine.prototype.infer = function(timeout, callback) {
    ...
    process.nextTick((function() {
        proxy._executeRule((function(err, newFacts) {
            if(err) {
                this._infering = false;
                clearTimeout(this.timeoutId);
                callback(err);
            ...
            if (newFacts && typeof newFacts === "object") {
                for (let factName in newFacts) {
                    proxy.set(factName, newFacts[factName]);
                }
            }
            if (this._infering) {
...
function parseParameters(rule) {
    let ruleParts = rule.toString()
        .split("=>")[0]
        .split(')')[0]
        .split('(');
    let ruleParameters = ruleParts[0]
    if (ruleParts.length > 1) {
        ruleParameters = ruleParts[1]
    }
 ...
 */
  ...
  *                                'addRule', 'infer', 'addToAgenda' or 'trace'.
  * @param {string} [trace#fromRule] - The rule name that generated the trace. This
