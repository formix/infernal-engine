...
module.exports = class InfernalEngine {
     ...
     * 
     * @param {Number} maxDepth Optional, default fo 50. The maximum number of agenda to dig down
     *                          while doing the inference. 
     * @param {Function} trace An optional tracing function that will be called with a trace
     *                         object parameter.
     */
    constructor(maxDepth, trace) {
    ...
    /**
     * Sets the fact to the given value and launche the inference.
     * 
     * @param {String} path The full path to the desired fact.
    ...
    async set(path, value) {
        ...
        if (!(value instanceof Array) && infernalUtils.equals(prevValue, value)) {
            return;
        }
...
async function _import(obj, context) {
    ...
    if (obj instanceof Date || obj instanceof Array) {
        return await this.set(targetContext, obj);
    }
    ...
    if (objtype === "function") {
        return await this.addRule(targetContext, obj);
    }
