...
Infernal.prototype.get = function(factName) {
    var absolutePath = getAbsolutePath(this._scope, factName);
    var fact = digPath.call(this, this._facts, absolutePath);
    if (fact === undefined) {
        return undefined;
    }
...
Infernal.prototype.set = function(factName, value) {
    ...
    if (this.get(factName) !== value) {
        var fact = digPath.call(this, this._facts, absolutePath, true);
        fact.target[fact.name] = value;
        if (this._relations[absolutePath] !== undefined) {
            var rules = this._relations[absolutePath];
...
Infernal.prototype.getFacts = function() {
    return deepCopy(this._facts);
};


Infernal.prototype.setFacts = function(facts) {
...
function applyFacts(context, source) {
    ...
    if (sourceType === "object") {
        for (var property in source) {
            applyFacts.call(this, context + "/" + property, source[property]);
        }
    } else {
