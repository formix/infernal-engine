...
function getContext(currentContext, path) {
    ...
    if (path.indexOf("/") === 0) {
        ...
        if (!currentContext || currentContext === "/" || currentContext.indexOf("/") !== 0) {
            // Makes sure that we have the empty item as the first element of 
            // the array to represents the root 'contextPart'.
            contextStack.push("");
        } else {
            contextStack = contextStack.concat(currentContext.split("/"));
        }
        contextStack = contextStack.concat(path.split("/"));
    }
