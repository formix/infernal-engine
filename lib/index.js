...
InfernalEngine.prototype.addRule = function(ruleName, rule) {
    ...
    let parameters = parseParameters(rule);
    let context = utils.getContext("/", ruleName);
    for (const param of parameters) {
        let factName = utils.getFullName(context, param);
...
function* parseParameters(rule) {
    ...
    let ruleCode = (rule.toString().split(')')[0] + ')');
    let ruleCodeWithoutEol = ruleCode.replace(/\s+/g, " ");
    let allParamMatch = allParamsRegex.exec(ruleCodeWithoutEol);
    ...
    while (paramMatch) {
        if (!firstParam) { // skip the first callback parameter
