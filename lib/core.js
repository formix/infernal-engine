...
function Engine(compil) {
    ...
    this.wildRelations = {} // Map between wild facts and all related rules
	...
	this.addRule = function(ruleName, rule) {
		...
		while (match) {
            ...
            if (factName.indexOf("*") === -1) {
                ...
                var wildFactName = factName
                    .replace(".", "\\.")
                    .replace("*", "(.*)");
                if (this.wildRelations[wildFactName] === undefined) {
                    this.wildRelations[wildFactName] = {};
                }
                this.trace( function() { 
                    return "(AE) Adding rule '" + ruleName + 
                        "' to the engine *wild* relations."; 
                });
                this.wildRelations[wildFactName][ruleName] = true;
            }
