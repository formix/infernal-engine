...
function execRules(engine, currentRules, info, callback) {  // jshint ignore:line
	...
	// each rule must execute within 5 seconds.
...
exports.engine = function(compil) {
	...
	var newEngine = {
		...
		relations : {}, // Map between factnames and all related rules
		...
		addRules : function(ruleSet) {
			for ( var ruleName in ruleSet) {
				if (ruleSet.hasOwnProperty(ruleName)) {
					this.addRule(ruleName, ruleSet[ruleName]);
				}
			}
		},
